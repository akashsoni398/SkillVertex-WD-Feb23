...
if(isset($_POST['submit'])) {
  ...
  if($email!="" && $pwd!="") {
    ...
    if($row['usercount'] > 0) {
      $sql_query = "SELECT id as userid,name AS username,password AS pwdcrypt FROM users WHERE email='$email'";
      $result = mysqli_query($conn,$sql_query);
      $row = mysqli_fetch_array($result);
      $pwdcrypt = $row['pwdcrypt'];
      if(password_verify($pwd,$pwdcrypt)) {
        $_SESSION['userid'] = $row['userid'];
        $_SESSION['username'] = $row['username'];
        header("Location:./../index.php");
      }
      else {
        $errmsg = "Usename and password did not match";
      }
    }
    else {
